...
BEGIN
  WITH flat AS (
    SELECT
      -- Flatten: merge sub-objects into top level
      item
  ...
  computed AS (
    SELECT
      e.*,
      -- Clean job_id: remove -GHOST suffix
      TRIM(SPLIT_PART(e.raw_id, '-GHOST', 1)) AS clean_id,
      -- Calculate fingerprint (getCaseBody logic)
      CASE
      ...
      CASE
        ...
        ELSE 0
      END AS hours_num,
      -- Parse raw wage value (remove $ and ,)
      CASE
  ...
  final_data AS (
    SELECT
      ...
      CASE
        ...
        ELSE c.wage_num
      END AS salary,
      -- dates with safe cast
      (CASE WHEN c.raw_start_date ~ '^\d{4}-\d{2}-\d{2}' THEN c.raw_start_date::date
            WHEN c.raw_start_date ~ '^\d{2}/\d{2}/\d{4}' THEN TO_DATE(c.raw_start_date, 'MM/DD/YYYY')
            ELSE NULL END) AS start_date,
      COALESCE(
        (CASE WHEN c.raw_posted_date ~ '^\d{4}-\d{2}-\d{2}' THEN c.raw_posted_date::date
              WHEN c.raw_posted_date ~ '^\d{2}/\d{2}/\d{4}' THEN TO_DATE(c.raw_posted_date, 'MM/DD/YYYY')
              ELSE NULL END),
        v_today
      ) AS posted_date,
      (CASE WHEN c.raw_end_date ~ '^\d{4}-\d{2}-\d{2}' THEN c.raw_end_date::date
            WHEN c.raw_end_date ~ '^\d{2}/\d{2}/\d{4}' THEN TO_DATE(c.raw_end_date, 'MM/DD/YYYY')
