...
BEGIN
  ...
  ON CONFLICT (fingerprint) DO UPDATE SET
    -- CORREÇÃO: preserva a data de postagem mais antiga (não sobrescreve com fallback de hoje)
    posted_date = LEAST(EXCLUDED.posted_date, public_jobs.posted_date),
...
-- Mesma correção no process_jobs_bulk (importador manual)
CREATE OR REPLACE FUNCTION public.process_jobs_bulk(jobs_data jsonb)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
BEGIN
    INSERT INTO public_jobs (
        job_id, fingerprint, visa_type, job_title, company, email, phone, 
        city, state, zip_code, salary, posted_date, start_date, end_date, 
        job_duties, job_min_special_req, wage_additional, rec_pay_deductions, 
        weekly_hours, category, openings, experience_months, education_required, 
        transport_provided, source_url, housing_info, was_early_access, is_active
    )
    SELECT 
        (val->>'job_id'), (val->>'fingerprint'), (val->>'visa_type'), (val->>'job_title'), 
        (val->>'company'), (val->>'email'), (val->>'phone'), (val->>'city'), 
        (val->>'state'), (val->>'zip_code'), (val->>'salary')::NUMERIC, 
        (val->>'posted_date')::DATE, (val->>'start_date')::DATE, (val->>'end_date')::DATE, 
        (val->>'job_duties'), (val->>'job_min_special_req'), (val->>'wage_additional'), 
        (val->>'rec_pay_deductions'), (val->>'weekly_hours')::NUMERIC, (val->>'category'), 
        (val->>'openings')::INT, (val->>'experience_months')::INT, (val->>'education_required'), 
        (val->>'transport_provided')::BOOLEAN, (val->>'source_url'), (val->>'housing_info'), 
        (val->>'was_early_access')::BOOLEAN, (val->>'is_active')::BOOLEAN
    FROM jsonb_array_elements(jobs_data) AS val
    ON CONFLICT (fingerprint) DO UPDATE SET
        -- CORREÇÃO: preserva a data mais antiga
        posted_date = LEAST(EXCLUDED.posted_date, public_jobs.posted_date),
        job_title = EXCLUDED.job_title,
        company = EXCLUDED.company,
        salary = EXCLUDED.salary,
