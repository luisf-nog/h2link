-- Drop the old single-parameter version to avoid ambiguity
DROP FUNCTION IF EXISTS public.track_profile_view(uuid);

-- Recreate only the two-parameter version (which handles null p_queue_id)
