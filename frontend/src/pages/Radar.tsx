...
const JobCard = ({ job, match, onApply, onView, onDismiss }: any) => (
  <div className="group relative rounded-2xl border border-border bg-card hover:border-primary/30 hover:shadow-md transition-all duration-300 p-5 space-y-4">
    ...
    <div className="flex items-start justify-between gap-3">
      ...
      <span className="text-[9px] text-muted-foreground/45">Detected 2m ago</span>
    </div>
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-12 p-6 md:p-12 animate-in fade-in duration-700">
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  {/* Title + Premium Access badge (premium gold, subtle pulse) */}
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={cn(
                        ...
                        "bg-gradient-to-b from-amber-200/15 via-amber-100/10 to-transparent",
                        "text-amber-200/90",
                        "shadow-[0_0_0_1px_rgba(255,215,130,0.06),0_12px_32px_-20px_rgba(255,200,80,0.35)]",
                      )}
                      aria-label={t("radar.premium_access", "Premium Access")}
                      title={t("radar.premium_access", "Premium Access")}
                    >
                      {/* subtle pulse dot */}
                      <span className="relative flex h-1.5 w-1.5">
                        <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-amber-200/35" />
                  ...
                  {/* Keep subtitle line (avoid changing existing copy patterns; add i18n fallback) */}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                {minWage ? `$${formatNumber(Number(minWage))}+/hr` : "Any salary"}
              </button>
              <button
                onClick={() => setShowFilters(true)}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-border bg-card text-xs font-semibold text-foreground hover:border-primary/30 hover:bg-primary/5 transition-colors shadow-sm"
              ...
              >
                ...
                {maxExperience ? `Up to ${maxExperience}mo exp` : "Any experience"}
              </button>
              <button
                onClick={() => setShowFilters(true)}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-border bg-card text-xs font-semibold text-foreground hover:border-primary/30 hover:bg-primary/5 transition-colors shadow-sm"
              >
                ...
                {visaType !== "all" ? visaType : "All Visas"}
              </button>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <HeroPanel className="p-0 overflow-hidden">
          <MetricCard
            ...
            icon={Activity}
            subtitle="Opportunities detected"
          />
        </HeroPanel>
        <HeroPanel className="p-0 overflow-hidden">
          <MetricCard
            ...
            icon={Target}
            subtitle={`${formatNumber(queuedFromRadar)} sent to queue`}
          />
        </HeroPanel>
        <HeroPanel className="p-0 overflow-hidden">
          <MetricCard
      ...
      <div className="grid grid-cols-1 lg:grid-cols-7 gap-8">
        {/* Left: Targeting Configuration — with subtle background differentiation */}
        <div className="lg:col-span-4 space-y-6">
          ...
          {/* Subtle background wrapper for the left column */}
          ...
          <div className="px-8 py-6 space-y-6">
            ...
            <div className="space-y-3">
              <p className="text-sm text-foreground leading-relaxed">
                O Radar escaneia <span className="font-bold text-primary">diariamente</span> todas as novas vagas
                publicadas no sistema DOL. Quando uma vaga compatível com seus filtros é detectada, ele age
            ...
            <div className="space-y-3 pt-2">
              <p className="text-[10px] font-bold text-muted-foreground/50 uppercase tracking-[0.18em]">Benefícios</p>
