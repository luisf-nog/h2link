...
export default function Auth() {
  ...
  const inputCls =
    "bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400 focus:border-sky-500 focus:ring-sky-500/20 h-11 rounded-xl w-full px-3 text-sm transition-all";
  const selectCls = `${inputCls} appearance-none`;
  const btnPrimary = `w-full h-12 rounded-xl text-white font-bold flex items-center justify-center gap-2 transition-all mt-6 shadow-sm ${primaryColor}`;
  const btnGhost =
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans selection:bg-sky-500/30">
      ...
      <AlertDialog open={errorDialog.open} onOpenChange={(open) => setErrorDialog((p) => ({ ...p, open }))}>
        <AlertDialogContent className="border-destructive/40 shadow-2xl bg-white">
          <AlertDialogHeader className="space-y-0 text-left">
            <div className="flex items-start gap-3">
              <div className="mt-0.5 inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-red-50 text-red-500">
                <AlertTriangle className="h-5 w-5" />
              </div>
              <div className="min-w-0">
                <AlertDialogTitle className="text-base font-bold text-slate-900">{errorDialog.title}</AlertDialogTitle>
          ...
          <AlertDialogFooter>
            <AlertDialogAction className="bg-red-500 text-white hover:bg-red-600 rounded-lg">
              {okLabel}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <div className="flex-1 flex flex-col relative overflow-y-auto">
        ...
        <div className="flex-1 flex items-center justify-center p-6 sm:p-12 mt-16 lg:mt-0">
          <div className="w-full max-w-[440px] bg-white p-8 sm:p-10 rounded-3xl border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            {/* Tabs */}
            <div className="flex p-1 bg-slate-100 rounded-xl mb-8">
              <button
                ...
                className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${tab === "signin" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
              >
                Sign In
              </button>
              <button
                onClick={() => setTab("signup")}
                className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${tab === "signup" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
              >
                Sign Up
              </button>
            </div>
                    ...
                    <form onSubmit={handleSignIn} className="space-y-5">
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-1.5">
                          <label className="text-[11px] font-bold uppercase tracking-wider text-slate-500 m-0">
                            Password
                          </label>
                          <button
                            type="button"
                      ...
                      <button
                        type="submit"
                        disabled={isLoading}
                        className={`w-full h-12 rounded-xl text-white font-bold flex items-center justify-center gap-2 transition-all mt-6 shadow-sm bg-[#0ea5e9] hover:bg-[#0284c7]`}
                      >
                  ...
                  <div className="animate-in fade-in zoom-in-95 duration-300">
                    ...
                    <form onSubmit={handleRequestPasswordReset} className="space-y-5">
                      ...
                      <button
                        type="submit"
                        disabled={isLoading}
                        className={`w-full h-12 rounded-xl text-white font-bold flex items-center justify-center gap-2 transition-all shadow-sm bg-[#0ea5e9] hover:bg-[#0284c7]`}
                      >
            ...
            {tab === "signup" && signupRole === "worker" && (
              <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-slate-900">Worker Account</h2>
                ...
                {workerStep === 2 && (
                  <form onSubmit={handleWorkerFinalSubmit} className="space-y-5">
                    ...
                    <div className="border border-slate-200 rounded-xl p-4 bg-slate-50 space-y-3 mt-2">
                      <p className="text-[10px] text-slate-500 leading-relaxed">{t("auth.disclaimer")}</p>
                      <div className="flex gap-2.5 items-start">
                        <Checkbox
                          id="accept-wrk"
                          checked={acceptTerms}
                          onCheckedChange={(v) => setAcceptTerms(v === true)}
                          className="border-slate-300 data-[state=checked]:bg-[#D4500A] data-[state=checked]:border-[#D4500A] mt-0.5"
                        />
                        <label
                          htmlFor="accept-wrk"
                          className="text-[11px] text-slate-600 leading-snug cursor-pointer font-medium"
                        >
                          {t("auth.accept_terms")}
                        </label>
                      </div>
            ...
            {tab === "signup" && signupRole === "employer" && (
              <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="flex items-center gap-3 mb-6">
                  <button
                    onClick={() => {
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-slate-900">Employer Account</h2>
                    <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mt-0.5">
                ...
                <div className="w-full h-1.5 bg-slate-100 rounded-full mb-8 overflow-hidden">
                  <div
                    className={`h-full ${primaryColor} transition-all duration-500`}
                    style={{ width: `${(employerStep / 3) * 100}%` }}
                  />
                </div>
                ...
                {employerStep === 3 && (
                  <form onSubmit={handleEmployerSubmit} className="space-y-5">
                    ...
                    <div className="border border-slate-200 rounded-xl p-4 bg-slate-50 space-y-3 mt-2">
                      ...
                      <div className="flex gap-2.5 items-start">
                        <Checkbox
                          ...
                          checked={acceptTerms}
                          onCheckedChange={(v) => setAcceptTerms(v === true)}
                          className="border-slate-300 data-[state=checked]:bg-[#0ea5e9] data-[state=checked]:border-[#0ea5e9] mt-0.5"
                        />
                        <label
                          htmlFor="accept-emp"
                          className="text-[11px] text-slate-600 leading-snug cursor-pointer font-medium"
                    ...
                    <div className="flex gap-3 mt-6">
                      ...
                      <button type="submit" disabled={isLoading} className={btnPrimary} style={{ marginTop: "12px" }}>
                        {isLoading ? <Loader2 size={18} className="animate-spin" /> : "Create Account"}
                      </button>
                    </div>
                  </form>
            ...
            {/* Micro-footer for the right panel */}
            <div className="mt-8 pt-6 border-t border-slate-100 flex justify-center">
              ...
              >
                {t("auth.browse_jobs_link", "Explore available jobs")}
              </button>
            </div>
          </div>
        </div>
