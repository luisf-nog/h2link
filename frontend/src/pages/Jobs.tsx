...
// --- COMPONENTE DE ONBOARDING (DESIGN PREMIUM & PODER DE AUTOMAÇÃO) ---
function OnboardingModal() {
  const [open, setOpen] = useState(false);

  useEffect(() => {
    // Versão 6 para garantir atualização de design para o usuário
...
export default function Jobs() {
  ...
  const addToQueue = async (job: Job) => {
    ...
    // Inserção simples no Supabase sem chamar funções de envio
    const { error } = await supabase.from("my_queue").insert({
      user_id: profile?.id,
      job_id: job.id,
      status: "pending", // Força o status inicial como pendente
    });
