...
import { 
  Bus, Calendar, Home, Mail, MapPin, MessageCircle, Phone, PhoneCall, Plus, Trash2, 
  Share2, AlertTriangle, Briefcase, Clock, DollarSign, Car, Shield, Weight, 
  FileCheck, Utensils
} from "lucide-react";
        ...
        <div className="flex-1 overflow-y-auto pr-2 -mr-2">
          <div className="space-y-6 py-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-muted/30 p-4 rounded-lg border">
              <div className="space-y-3">
                ...
                <div className="pl-7 space-y-1">
                  ...
                  {job?.wage_additional && (
                     <p className="text-xs text-muted-foreground mt-2 border-l-2 pl-2 border-primary/20">{job.wage_additional}</p>
                  )}
                </div>
              </div>
                ...
                <div className="pl-7 space-y-2 text-sm">
                  ...
                  {(job?.shift_start || job?.shift_end) && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{job?.shift_start || "?"} - {job?.shift_end || "?"}</span>
                    </div>
                  )}
                   <div className="flex justify-between">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm border-b pb-6">
               <div>
                  <span className="block text-muted-foreground text-xs uppercase">{t("job_details.fields.start_date")}</span>
                  <span className="font-medium">{formatDate(job?.start_date)}</span>
               </div>
               <div>
                  <span className="block text-muted-foreground text-xs uppercase">{t("job_details.fields.end_date")}</span>
                  <span className="font-medium">{formatDate(job?.end_date)}</span>
               </div>
               <div>
                  <span className="block text-muted-foreground text-xs uppercase">{t("job_details.fields.openings")}</span>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                 <div className={cn("flex flex-col items-center p-3 rounded border text-center gap-2", job?.job_is_lifting ? "bg-red-50 border-red-200" : "bg-slate-50")}>
                    <Weight className="h-5 w-5 text-muted-foreground" />
                    <span className="text-xs font-medium">Levantar Peso</span>
                    <span className="text-[10px] text-muted-foreground">{job?.job_is_lifting ? (job.job_lifting_weight || "Sim") : "Não"}</span>
                 </div>
                 
                 <div className={cn("flex flex-col items-center p-3 rounded border text-center gap-2", job?.job_is_driver ? "bg-blue-50 border-blue-200" : "bg-slate-50")}>
                 ...
                 <div className={cn("flex flex-col items-center p-3 rounded border text-center gap-2", job?.job_is_drug_screen ? "bg-yellow-50 border-yellow-200" : "bg-slate-50")}>
                    <FileCheck className="h-5 w-5 text-muted-foreground" />
                    <span className="text-xs font-medium">Anti-Drogas</span>
                    <span className="text-[10px] text-muted-foreground">{job?.job_is_drug_screen ? "Obrigatório" : "Não"}</span>
                 </div>

                 <div className={cn("flex flex-col items-center p-3 rounded border text-center gap-2", job?.experience_months ? "bg-green-50 border-green-200" : "bg-slate-50")}>
                    <Briefcase className="h-5 w-5 text-muted-foreground" />
                    <span className="text-xs font-medium">Experiência</span>
                    <span className="text-[10px] text-muted-foreground">{job?.experience_months ? `${job.experience_months} meses` : "Nenhuma"}</span>
                 </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-md border p-4 bg-slate-50 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="flex flex-col gap-1">
                   ...
                   {job?.website && (
                      <a href={job.website} target="_blank" rel="noreferrer" className="text-xs text-blue-600 hover:underline">
                   ...
                   {job?.phone && (
                     <>
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button size="icon" className="rounded-full h-10 w-10" asChild>
                                <a href={getSmsUrl(job.phone)!}><MessageCircle className="h-5 w-5" /></a>
                              </Button>
                            </TooltipTrigger>
                           ...
                           <Tooltip>
                            <TooltipTrigger asChild>
                              <Button size="icon" variant="secondary" className="rounded-full h-10 w-10" asChild>
                                <a href={getPhoneCallUrl(job.phone)!}><PhoneCall className="h-5 w-5" /></a>
                              </Button>
                            </TooltipTrigger>
                        ...
                        {isMobileNumber(job.phone) && getWhatsAppUrl(job.phone) && (
                           <Tooltip>
                            <TooltipTrigger asChild>
                              <Button size="icon" className="rounded-full h-10 w-10 bg-[#25D366] hover:bg-[#128C7E] text-white" asChild>
