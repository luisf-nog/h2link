...
export function JobDetailsDialog({
  ...
  const canSaveJob = isRegistered && !isSponsored;

  const hasValidPhone =
    !isSponsored && job?.phone && job.phone !== "N/A" && job.phone !== "n/a" && job.phone.trim() !== "" && job.phone !== "0";
      ...
      <DialogContent className="sm:max-w-7xl h-screen sm:h-auto max-h-[100dvh] flex flex-col p-0 gap-0 overflow-hidden rounded-none sm:rounded-lg border-0 sm:border text-left">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      className={cn(
                        ...
                        isSponsored && "bg-amber-100 border-amber-300 text-amber-800",
                        !isSponsored && job.visa_type === "H-2A" && !job.was_early_access && "bg-green-600 border-green-600 text-white",
                        !isSponsored && job.visa_type === "H-2B" && !job.was_early_access && "bg-blue-600 border-blue-600 text-white",
                        !isSponsored && (job.visa_type.includes("Early Access") || (job.was_early_access && isCurrentlyEarlyAccess)) &&
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50/30 touch-auto text-left">
          <div className="p-4 sm:p-6 space-y-6 pb-32 sm:pb-6">
            ...
            {!isSponsored && isCertifiedOpportunity && (
