...
export function JobDetailsDialog({ open, onOpenChange, job, planSettings, formatSalary, onAddToQueue }: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-7xl h-screen sm:h-auto max-h-[100dvh] flex flex-col p-0 gap-0 overflow-hidden border-0 sm:border text-left">
        <div className="p-4 sm:p-6 bg-white border-b sticky top-0 z-40 shrink-0">
          <div className="flex justify-between items-start">
            ...
            <div className="hidden sm:flex gap-2 shrink-0">
              <Button onClick={() => job && onAddToQueue(job)} className="px-6 font-bold" disabled={!planSettings}>
                {!planSettings && <Lock className="h-4 w-4 mr-2" />} {t("jobs.details.save_job")}
              </Button>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-8">
                <div className="bg-white p-6 rounded-xl border border-slate-200">
                  <h4 className="flex items-center gap-2 font-bold text-lg text-slate-800 mb-4 border-b pb-2">
                    <Briefcase className="h-5 w-5 text-blue-600" /> Descrição da Vaga
                  </h4>
