  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-7xl h-[95vh] sm:h-auto flex flex-col p-0 gap-0 overflow-hidden rounded-none sm:rounded-lg">
        {/* TOP HEADER FIXO (STICKY) */}
        <div className="p-4 sm:p-6 pb-4 bg-white border-b sticky top-0 z-30 shadow-sm flex flex-col gap-2">
          {/* Mobile Back Button */}
          <div className="flex sm:hidden items-center mb-2 -mt-2">
            <Button
              variant="ghost"
        ...
        {/* ÁREA DE CONTEÚDO ROLÁVEL (SCROLLABLE AREA) */}
        <div className="flex-1 overflow-y-auto bg-slate-50/30">
          <div className="p-4 sm:p-6 space-y-6">
            {/* O CARD DOURADO AGORA ROLA JUNTO COM O CONTEÚDO */}
            {job?.was_early_access && (
              <div
                className="rounded-lg border border-amber-100 bg-amber-50/60 overflow-hidden transition-all duration-300 cursor-pointer"
              ...
              >
                <div className="p-3 flex items-center justify-between">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <h4 className="text-sm font-bold text-amber-900 uppercase tracking-tight text-left">
                      {t("jobs.details.early_match.title")}
                    </h4>
                  </div>
                  <div className="flex items-center text-amber-600">
                    {!isBannerExpanded && (
                      <span className="text-[10px] font-bold mr-2 uppercase opacity-70">Ver mais</span>
                    )}
                    {isBannerExpanded ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                  </div>
                </div>
                {isBannerExpanded && (
                  <div className="px-4 pb-4 pt-1 animate-in fade-in slide-in-from-top-1 duration-200 border-t border-amber-100/50 text-left">
                    <p className="text-sm text-amber-800 leading-relaxed">{t("jobs.details.early_match.desc")}</p>
                  </div>
                )}
              </div>
            )}
        ...
        <div className="sm:hidden p-4 border-t bg-white flex gap-3 sticky bottom-0 z-30 shadow-lg">
